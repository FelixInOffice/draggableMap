...
for (var i = 0; i < trees.length; i++) {
    var x = (trees[i].x - 88) * 1.8;
    var y = (trees[i].y - 20) * 1.8;
    ...
    d.appendChild(img);
    document.getElementById('treeHere').appendChild(d);
    $('#' + d.id).attr('data', trees[i].id);
    $('#' + d.id).attr('curr', i);
    ...
    //     s.id = 'shadowd' + i;
    //     s.className += 'htmlMapTreeShadow';
    //     s.style.marginLeft = (x - 13) + 'px';
    //     s.style.marginTop = (y + 10) + 'px';
    //     document.getElementById('htmlMapBase').appendChild(s);

    //     /*
    //      * animation
    //      */

    //     var delay = 1000 + (randomId[i] / 4) * 85;
...
function update() {
    Draggable.create(".tree_box", {
        bounds: $tree_container,
        edgeResistance: 0.65,
        type: "x,y",
        throwProps: true,
        autoScroll: true,
